class Photo
  ...
  def self.all(offset=0, limit=0)
    mongo_client.database.fs.find.skip(offset).limit(limit).map { |val| Photo.new(val)}
  end

  def self.find(id)
    val = mongo_client.database.fs.find(:_id => BSON::ObjectId.from_string(id)).first
    val.nil? ? nil : photo = Photo.new(val)
  end
