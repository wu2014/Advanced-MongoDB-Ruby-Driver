class Place
  ...
  def self.find(s)
    ...
    p = collection.find(:_id => id).first
    if !p.nil?
      Place.new(p)
    else
      nil
  ...
  def self.find_ids_by_country_code(c)
    Place.collection.aggregate([
      {:$unwind => "$address_components"}, 
      {:$project =>{ :_id => 1, :address_components => {:short_name => 1, :types => 1} }}, 
      {:$match => {"address_components.short_name" => c}}
    ]).map {|h| h[:_id].to_s}
  end
  ...
  def self.near(p, max_meters=nil)
    ...
    if !max_meters.nil?
      Place.collection.find({"geometry.geolocation" => {'$near'=> p.to_hash, :$maxDistance => max_meters}})
    else
      Place.collection.find({"geometry.geolocation" => {'$near'=> p.to_hash}})
    end
  end
  ...
  def near(max_meters=nil)
    ...
    near_points = []
    if !max_meters.nil?
      Place.collection.find({"geometry.geolocation" => {'$near'=> @location.to_hash, :$maxDistance => max_meters}
        }).each { |p| near_points << Place.new(p) }
    else
      Place.collection.find({"geometry.geolocation" => {'$near'=> @location.to_hash}
        }).each { |p| near_points << Place.new(p) }
    end
  ...
  def photos(offset=0, limit=0)
    self.class.mongo_client.database.fs.find( "metadata.place" => BSON::ObjectId.from_string(@id)
